...
func (a *App) SaveTask(name string, url string, method string, ck string, data string,
	...
	if cronExpression != "" {
		...
		if err != nil {
			return fmt.Sprintf("任务已保存，ID: %s，但定时任务调度失败: %s", taskID, err.Error())
		}
	}
	...
	if err != nil {
		...
		a.mu.Unlock()
		return fmt.Sprintf("保存任务失败: %s", err.Error())
	}

	return fmt.Sprintf("任务保存成功，ID: %s", taskID)
}

// GetAllTasks returns all saved tasks with pagination support
func (a *App) GetAllTasks(page int, pageSize int) map[string]*Task {
	if page < 1 {
		page = 1
	}
	if pageSize < 1 {
	...
	tasksSlice := make([]*Task, 0, len(a.Tasks))
	...
	sort.Slice(tasksSlice, func(i, j int) bool {
		return tasksSlice[i].UpdatedAt > tasksSlice[j].UpdatedAt
	})

	// Calculate start and end indices
	start := (page - 1) * pageSize
	end := start + pageSize
	if start >= len(tasksSlice) {
		return make(map[string]*Task)
	}
	if end > len(tasksSlice) {
		end = len(tasksSlice)
	}

	// Create result map
	result := make(map[string]*Task)
	for _, task := range tasksSlice[start:end] {
		// 创建任务的副本以避免并发修改问题
		taskCopy := *task
		result[task.ID] = &taskCopy
	}
...
func (a *App) saveTasksToDisk() error {
	...
	// 创建临时文件
	tempFile := tasksPath + ".tmp"
	err = os.WriteFile(tempFile, tasksJSON, 0644)
	if err != nil {
		return fmt.Errorf("写入临时文件失败: %w", err)
	}

	// 原子性地重命名临时文件
	err = os.Rename(tempFile, tasksPath)
	if err != nil {
		// 如果重命名失败，尝试删除临时文件
		os.Remove(tempFile)
...
// LoadTaskLogs loads task logs from disk with pagination
func (a *App) LoadTaskLogs(taskID string, page int, pageSize int) *TaskLogsResult {
	if page < 1 {
		page = 1
	}
	if pageSize < 1 {
	...
	if taskID != "" {
		...
		if len(content) > 0 {
			...
			result.Logs[taskID] = logs[start:end]
		}
	} else {
		// Load logs for all tasks
...
// GetTotalLogs returns the total number of log entries for a task
func (a *App) GetTotalLogs(taskID string) int {
	logsPath := a.getLogsPath()
	if taskID != "" {
		taskLogPath := filepath.Join(logsPath, taskID+".log")
		content, err := os.ReadFile(taskLogPath)
