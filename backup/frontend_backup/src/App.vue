...
const loadTasks = async (page: number = currentPage.value, size: number = pageSize.value) => {
  try {
    ...
    if (!tasksData) {
      outputLog.value += getCurrentTime() + ` 加载任务列表失败: 返回数据为空\n`;
      return;
    }
    ...
    outputLog.value += getCurrentTime() + ` 成功加载 ${Object.keys(tasksData).length} 个任务\n`;
  } catch (error) {
...
const availableTags = computed(() => {
  try {
    ...
    console.error('availableTags计算错误:', error);
    return [];
  }
});

// 缓存过滤结果以避免频繁重新计算
let filteredTasksCache: any[] = [];
let lastFilterState: { filter: string; selectedTags: string[]; tasksCount: number } = {
  filter: '',
  selectedTags: [],
  tasksCount: 0
};
...
const filteredTasks = computed(() => {
  try {
    ...
    const stateChanged =
      lastFilterState.filter !== currentState.filter ||
      lastFilterState.selectedTags.length !== currentState.selectedTags.length ||
      lastFilterState.selectedTags.some((tag, index) => tag !== currentState.selectedTags[index]) ||
      lastFilterState.tasksCount !== currentState.tasksCount;

    if (!stateChanged && filteredTasksCache.length > 0) {
    ...
    const filtered = tasksArray.filter(task => {
      ...
      const matchesTags = selectedTags.value.length === 0 ||
        (task.tags && selectedTags.value.some(tag => task.tags.includes(tag)));
    ...
    // 更新缓存
    filteredTasksCache = filtered;
    lastFilterState = currentState;

    return filtered;
  } catch (error) {
    console.error('filteredTasks计算错误:', error);
    return [];
  }
});
...
const timeRemainingDisplay = computed(() => {
  ...
  // Only include timerTick dependency when timer is actually active
...
onMounted(async () => {
  ...
  clockInterval = setInterval(() => {
    ...
    // Only increment timer tick when timer is active to reduce unnecessary computations
    if (timerActive.value) {
      timerTick.value++;
    }
    checkScheduledTasks();
  }, 1000) as unknown as number; // Reduced frequency to 1 second

  // Load all saved tasks with proper pagination
